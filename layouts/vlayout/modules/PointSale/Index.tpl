		...
		var category = this.value;
			...
			AppConnector.request(params).then(
				...
				function(error) {
					...
					console.log("Very serious error. Investigate function getCatVal in Pointsale/Index.tpl");
				}	
			);
	});
	var dialog, form,
...
function search(ele) {
    if(event.key === 'Enter') {
		...
		AppConnector.request(params).then(
			...
			function(error) {
				//progressIndicatorElement.progressIndicator(hideparams);
				console.log("Very serious error. Investigate function search in Pointsale/Index.tpl");
			}	
		);
    }
}
...
function customProduct(id){
		...
		if($("#product"+productid).length == 0) { // If prduct is alrady selected
			...
			AppConnector.request(params).then(
				...
				function(error) {
					//progressIndicatorElement.progressIndicator(hideparams);
					console.log("Very serious error. Investigate function customProduct in Pointsale/Index.tpl");
				}	
			);	
...
function proceed(){
	...
	AppConnector.request(params1).then(
		...
		function(error) {
			console.log("Very serious error. Investigate function saveData in Pointsale/Index.tpl");
		}	
	);	
