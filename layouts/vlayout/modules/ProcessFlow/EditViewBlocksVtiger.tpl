	...
	<tbody>  
        ...
        {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
             {assign var="isReferenceField" value=$FIELD_MODEL->getFieldDataType()}
                ...
                {/if}
            {if $FIELD_NAME == 'pf_process_status'  }
                <input type='hidden' name='{$FIELD_NAME}_hidden' id="{$FIELD_NAME}_hidden" value="{$FIELD_MODEL->get('fieldvalue')}"> 
            {/if}
...
{/foreach}  
<input type="hidden" id="recordid" value="{if $RECORD_ID neq ""}true{else}false{/if}" />
<script>
