			...
			if (typeof multipickBlocks !== 'undefined') {
				if (Object.keys(multipickBlocks).length > 0) {
					jQuery.each(multipickBlocks, function(index, value){
						pickkey = jQuery("#Products_detailView_fieldValue_"+index).find("span").first().html().trim();
						selectedBlockId = "";
						...
						jQuery("#Products_detailView_fieldValue_"+index).change(function(){
							selectedBlockId = "";
							selectedKey = jQuery(this).closest('td').attr('id').split("Products_detailView_fieldValue_")[1];
							pickkey = jQuery("#Products_detailView_fieldValue_"+selectedKey).find("a.chzn-single").find('span').first().text().trim();
