...
getcounterlists : function() {
	...
	showIssuesList : function(page,recordid,submodule)
     {
		 ...
		 var search_changedon	=	jQuery('input[name=search_changedon]').val();
			...
			function(error) {
			consile.log("Very serious error. Investigate!!");
		}
	...
	getModuleData : function(page){
			 jQuery("#picklistFilter").on('change',function(){
				var modulename = $(this).val();
				 if(modulename!=''){
					 jQuery('input[name=search_modulename]').val('');
				 	jQuery('input[name=search_modulename]').attr("disabled","disabled");
					 var search_modulename	=	'';
				 }
				 else{
					 jQuery('input[name=search_modulename]').removeAttr("disabled");
					 jQuery('input[name=search_modulename]').val('');
				 }jQuery('input[name=search_username]').val('');
				 var recordid	=	jQuery('#recordid').val();
				 jQuery('input[name=search_fieldname]').val('');
				 jQuery('input[name=search_prevalue]').val('');
				 jQuery('input[name=search_postvalue]').val('');
				 jQuery('input[name=search_changedon]').val('');
				 var modulename = $("#picklistFilter").val();
				  var currentLocation = 'index.php?module=OS2LoginHistory&parent=Settings&view=Detail&record='+recordid+'&submodule='+modulename;
	...
	registerDetailSearch : function() {
      ...
      jQuery('[data-trigger="listSearch"]').on('click',function(e){
		  ...
		  var search_changedon	=	jQuery('input[name=search_changedon]').val();
		 var modulename = $("#picklistFilter").val();
		  var currentLocation = 'index.php?module=OS2LoginHistory&parent=Settings&view=Detail&record='+recordid+'&search_modulename='+search_modulename+'&search_fieldname='+search_fieldname+'&search_prevalue='+search_prevalue+'&search_postvalue='+search_postvalue+'&search_changedon='+search_changedon+ '&submodule='+modulename;
		  window.location.href=currentLocation;
