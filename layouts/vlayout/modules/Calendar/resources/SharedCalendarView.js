...
var progressIndicatorElement = jQuery.progressIndicator({
	...
	saveUserCalendar : function(data, currentEle) {
		...
		AppConnector.request(params).then(function() {
			...
			//Started here auto refresh :: Sep 6th 2018				
				var params = {
					module:'Calendar',
					view:'ViewTypes',
				 ...
				 AppConnector.request(params).then(function(data){
					jQuery("#Calendar_sideBar_LBL_ADDED_CALENDARS").html(data);
				});
				//Ended here auto refresh
	...
	performCalendarFeedIntiate : function() {
		...
		//Check box for all :: 23 Aug 2018
		this.registerCheckAllusers();
	},
	//Check box for all :: 23 Aug 2018 added by manasa
	registerCheckAllusers: function(){
		var thisInstance=this;
		jQuery("#alluserscheck").attr('checked',true); 
		jQuery("#alluserscheck").on('click',function(e){
			var currentTarget = jQuery(e.currentTarget);
			var checkstatus  = currentTarget.is(':checked');
			var calendarfeeds = jQuery('[data-calendar-feed]');		
			if(checkstatus){				
				//alert(currentTarget.is(':checked'));
				calendarfeeds.each(function(index,element){
					var feedcheckbox = jQuery(element);
					var result = {};
					feedcheckbox.attr('checked',true); 
				...
				calendarfeeds.each(function(index,element){
					...
					feedcheckbox.attr('checked',false);	
					//Remove the events of deleted user in shared calendar feed
					thisInstance.getCalendarView().fullCalendar('removeEventSource', thisInstance.calendarfeedDS[feedcheckbox.data('calendar-sourcekey')]);					
				});
			}
