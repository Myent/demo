...
{/literal}
<div class="dashboardHeading" style="padding: 0px 0 0;background: #f5f5f5;margin-bottom:10px;">
	<div class="row-fluid">
		<div class="span5">
			...
			<div class="pull-right">
				<div class="btn-toolbar">
					<span class="btn-group">
						<button style="margin-right: 20px;" class="btn btn-danger" id='cancelBtn'>Cancel</button>
					</span>
				</div>
			</div>
		</div>
...
<div class="gridster ready" style="" >
	...
	<div class="row-fluid" id='cartBlock' style="margin-bottom: -26px;">

		<div class="span6">
			<div class="span4 offset1" style='float:left;padding-bottom:14px;'><center><h3>BILL OF SALE</h3></center></div>
			<div class="span8" style="overflow: auto;max-height:400px; border: 1px solid #dddddd;height:400px;">
			...
			<div class="span8" style="border: 1px solid #dddddd; height:50px;">
				<span class="span">
          <span id='contactBtn' class="btn addButton summaryImg" style="margin-top:9px;"><i class="icon-user" style="vertical-align: middle;padding: 2px;"></i>Add Customer</span>
				...
				<button style="float:right;margin-top:10px;margin-right:10px;display:none;" class="btn btn-success" id='paymentBtn'>Payment</button>

			</div>
		</div>


		<div class="span6" id='productsBlock' >
			<div class="span2" style='float:left;'><center><h3>PRODUCTS</h3></center></div>
        ...
        <div class="input-prepend">
          <span class="add-on"><i class="icon-barcode" title="Enter Barcode"></i></span>
          <input type='text' name='barcode_num' value="" onkeydown="search(this)" style="width:175px;"/>
        </div>
			</div>
			<div class="span8" style="background-color:lavender; height:450px; max-height:450px;overflow: auto;border: 1px solid #b1adad;">
				<div id='ajaxLoadP'>
				{foreach from=$ALLPRODUCTS key=k item=data}
					...
					<span class="span" data-product-id="{$data['productid']}">
						...
						{if $data['path'] eq ''} src="layouts/vlayout/modules/OS2PointSale/no-image.jpg" {else} src="{$data['path']}{$data['attachmentsid']}_{$data['name']}"{/if} height="50" width="50" {if $qty <= 0.00} style="border-color:red;border-width:1px;border-style:solid;height:50px;" {else} style="height:50px;" {/if} >
		...
		<div id='paymentBlock' style='display:none;' class="span6">
			<center><h3>PAYMENT</h3></center>
			<div class="span8">
				<table class="table table-striped table-bordered" style="margin-top: 10px;">
					<thead>
						<tr>
							<td><button class="btn btn-danger" id='backBtn' type="button">Back</button></td>
							<td><button class="btn btn-success" style="float:right;display:none;" id='proceed' onclick="proceed();" type="button">Proceed</button></td>
						</tr>
					</thead>
					<tbody id='tblbody1'>
						<tr>
							<td><button class="btn btn-primary" type="button" id='cashBtn'>CASH</button></td>
	...
	<div class="span8 offset3" id='receiptBLock' style="text-align: center;display:none;">
        ...
        <div id="receipt_print">
          <span style="float:left;"><button class="btn btn-danger" type="button" onclick="location.href='index.php?module=OS2PointSale&view=List'" >Exit</button></span>
        </div>
				<div id='receipt_no' style="border-bottom: 1px solid #ccc;">
			...
			AppConnector.request(params).then(
				function(data) {
					if (data.success){
						...
						var image = 'layouts/vlayout/modules/OS2PointSale/no-image.jpg';
				...
				function(error) {
					...
					console.log("Very serious error. Investigate!!");
				}
			);
	});
	var dialog, form,
		...
		function addUser() {
			...
			allFields.removeClass( "ui-state-error" );
				$.ajax({
					url: "index.php?module=OS2PointSale&action=Create&name="+name.val()+"&mobile="+mobile.val()+"&state="+state.val()+"&city="+city.val()+"&country="+country.val(),
					dataType: 'json',
...
function proceed(){
	...
	AppConnector.request(params1).then(
		function(data) {
			$('#receipt_print').append("<span style='float:right;'><button class='btn btn-success' type='button' onclick='downloadPopup("+data.result.id+");'>Print Receipt</button></span>");
			$("#receipt_no").append("<div class=''><p><strong></strong></p></div><div class=''><p><em>Date: "+data.result.date+"</em></p><p><em>Receipt #: "+data.result.sale+"</em></p></div>");
	...
	function downloadPopup(id){
		var URL = 'index.php?module=OS2PointSale&view=Print&mode=GetPrintReport&record='+id;
