...
SO_addressFieldsMapping : {'Contacts' :
    ...
    registerAddingNewProductsAndServices: function(){
		...
		/*Ganesh Added on 01 SEP 2018 Start*/
		jQuery('#openListEvent').on('click',function(){			
			...
			if(cont != 0){
				...
				AppConnector.request(subProrductParams).then(
					function(data){
						jQuery("#resulttimetrack").html(data);
						jQuery('#timetrackermodal').modal('show');
					}
				);
			}
		...
		jQuery('#btnSaveEvent').on('click',function(){
			...
			jQuery('input[type="checkbox"]:checked').each(function() {
				...
				var activitytype = jQuery('#activitytype'+actid).val();
				var activityqty = jQuery('#activityqty'+actid).val();
				var activityprice = jQuery('#activityprice'+actid).val();
				var totalprice = jQuery('#totalprice'+actid).val();
				var totalAfterDiscount = jQuery('#totalAfterDiscount'+actid).val();
				var serviceid = jQuery('#serviceid'+actid).val();
				var newRow = thisInstance.getBasicRow().addClass(thisInstance.rowClass);
				...
				newRow.find('input.productName').addClass('autoComplete');
				thisInstance.registerLineItemAutoComplete(newRow);
				newRow.find('input.productName').val(activitytype);
				jQuery('#productName'+sequenceNumber).attr('disabled', 'disabled');
				jQuery('#qty'+sequenceNumber).val(activityqty);
				jQuery('#productTotal'+sequenceNumber).text(totalprice);
				jQuery('#totalAfterDiscount'+sequenceNumber).text(totalAfterDiscount);
				jQuery('#netPrice'+sequenceNumber).text(totalAfterDiscount);
				jQuery('#hdnProductId'+sequenceNumber).val(serviceid);
				jQuery('#lineItemType'+sequenceNumber).val('Services');
				jQuery('#listPrice'+sequenceNumber).val(activityprice);
				jQuery('#listPrice'+sequenceNumber).focus();
				//aDeferred.resolve();
			});
			jQuery('#timetrackermodal').modal('hide');
		});
		/*Ganesh Added on 01 SEP 2018 End*/
