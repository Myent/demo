	...
	<ul class="liStyleNone">
		...
		{if is_array($PARENT_COMMENTS)}
			{foreach key=Index item=COMMENT from=$PARENT_COMMENTS}
				...
				{include file='Comment.tpl'|@vtemplate_path:'Calendar' COMMENT=$COMMENT COMMENT_MODULE_MODEL=$COMMENTS_MODULE_MODEL}
				...
				{if $CHILDS_ROOT_PARENT_MODEL}
					{if $CHILDS_ROOT_PARENT_MODEL->getId() eq $PARENT_COMMENT_ID}		
						...
						{include file='CommentsListIteration.tpl'|@vtemplate_path:'Calendar' CHILD_COMMENTS_MODEL=$CHILD_COMMENTS_MODEL}
					{/if}
				{/if}
				</li><br>	
			{/foreach}
		{else}
			{include file='Comment.tpl'|@vtemplate_path:'Calendar' COMMENT=$PARENT_COMMENTS}
		{/if}
	</ul>
{else}
