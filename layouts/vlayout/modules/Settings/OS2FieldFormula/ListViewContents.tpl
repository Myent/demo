...
<div class="listViewEntriesDiv" style='overflow-x:auto;'>
	...
	<table class="table table-bordered table-condensed listViewEntriesTable">
		<thead>
			...
			<th></th>
			<th></th>
            <th class='listViewEntries'>
				<div class="row-fluid">
					<select class="select2 listSearchContributorformula span12" name="modulename" multiple style="width:150px;" >
					 {foreach item=MODULE_NAME from=$SUPPORTED_MODULES}
							<option value="{$MODULE_NAME}" 
							{if in_array($MODULE_NAME,$SEARCH_VALUES) && ($MODULE_NAME neq "") } selected{/if}>
			...
			<th></th>
			 ...
			 <th class='listViewEntries'>
				<div class="row-fluid">
					<select class="select2 listSearchContributorformula span6" name="formulastatus" multiple style="width:150px;" >
						<option value="Active" 
							{if in_array('Active',$SEARCH_VALUES) && ('Active' neq "") } selected{/if}>
							Active
						</option>
						<option value="InActive" 
							{if in_array('InActive',$SEARCH_VALUES) && ('InActive' neq "") } selected{/if}>
							InActive
						</option>
					</select>
				</div>
			...
			<th>
				<button class="btn formulasearch">{vtranslate('LBL_SEARCH', $MODULE )}</button>
			</th>
        </tr>
		</thead>
		<tbody id="searchdatareplace">
			...
			<td width="1%" nowrap class="{$WIDTHTYPE}">
				...
				{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
					...
					<td class="listViewEntryValue {$WIDTHTYPE}"  width="{$WIDTH}%" nowrap>
						{assign var=fval value=$LISTVIEW_ENTRY->getDisplayValue($LISTVIEW_HEADERNAME)}
							{assign var=display value=$fval|replace:'#':''}
							{if $LISTVIEW_HEADERNAME == 'resultfield'}
								{assign var=listmodule  value=$LISTVIEW_ENTRY->getDisplayValue('module')}
								&nbsp;{vtranslate_label($display,$listmodule)}
								
							{else}
								{vtranslate($display, $QUALIFIED_MODULE)}
							{/if}
